...
class DownsampleObsLDM(pl.LightningModule):
    def __init__(
        ...
        # 2025-04-14: Currently no joint attention encoder available 
        # del ae_config['joint_encoder_kwargs']
        # print(ae_config.keys())
        ...
        self.obs_down = nn.Linear(1024, hidden_size)
        ...
        if ckpt_path is not None:
            ...
            if hasattr(autoencoder, 'lang_tokenizer'):
                self.lang_tokenizer = copy.deepcopy(autoencoder.lang_tokenizer)
            else: 
                raise NotImplementedError('Language tokenizer could not be found')
            if hasattr(autoencoder, 'lang_emb_model'):
                self.lang_emb_model = copy.deepcopy(autoencoder.lang_emb_model)
            else:
                raise NotImplementedError('Language embedding model could not be found')
            for p in self.lang_emb_model.parameters():
                p.requires_grad = True
    ...
    def pred_epsilon(self, noise, timestep, language_emb, ldm_obs_emb):
        ...
        # 2025-04-14: Currently no language embeddings available 
        embeddings = torch.cat([time_emb, language_emb, ldm_obs_emb, self.z_up(noise)], dim=1)
        # ldm_obs_emb = ldm_obs_emb.squeeze(-1).squeeze(-1).unsqueeze(1)
        # embeddings = torch.cat([time_emb, ldm_obs_emb, self.z_up(noise)], dim=1)
    ...
    def predict_action(self, raw_language_features, raw_image_features, raw_low_dim_data=None):
        ...
        assert raw_image_features.shape[1]%3 == 0, 'Given number of channels is not a multiple of 3!'
        if raw_image_features.shape[1] == 3: 
            ...
            n_images = int(raw_image_features.shape[1]/3)
            ldm_obs_embs, ae_obs_embs = [], []
            for i in range(n_images): 
                raw_image_feature=raw_image_features[:, i*3:(i+1)*3, :, :]
                ldm_obs_emb =  self.get_obs_emb(raw_image_features=raw_image_feature, raw_low_dim_data=raw_low_dim_data)
                ae_obs_emb = self.autoencoder.get_obs_emb(raw_image_features=raw_image_feature, raw_low_dim_data=raw_low_dim_data)
                ldm_obs_embs.append(ldm_obs_emb)
                ae_obs_embs.append(ae_obs_emb)
                
            ldm_obs_emb = torch.cat(ldm_obs_embs, dim=1)
            ae_obs_emb = torch.cat(ae_obs_embs, dim=1)
            ldm_obs_emb = self.obs_down(ldm_obs_emb).unsqueeze(1)
            ae_obs_emb = self.autoencoder.obs_down(ae_obs_emb).unsqueeze(1)
        ...
        batch_size = raw_image_features.shape[0]

        # ldm_obs_emb = ldm_obs_emb.squeeze(-1).squeeze(-1).unsqueeze(1)
        # ae_obs_emb = ae_obs_emb.squeeze(-1).squeeze(-1).unsqueeze(1)
        # recover z
        z = torch.randn(size=(batch_size, 1, self.latent_size), dtype=ldm_obs_emb.dtype, device=ldm_obs_emb.device)
    ...
    def forward(self, batch, batch_idx, split='train'):
        posterior, _ = self.autoencoder.encode(batch) # from pretrained vae
        
        image = batch['image']
        assert image.shape[1]%3 == 0, 'Given number of channels is not a multiple of 3!'
        if image.shape[1] == 3: 
            ldm_obs_emb = self.get_obs_emb(raw_image_features=image, raw_low_dim_data=batch.get('low_dim'))
            ldm_obs_emb = ldm_obs_emb.squeeze(-1).squeeze(-1).unsqueeze(1)
        else: # more than one camera view/ more than three channels 
            n_images = int(image.shape[1]/3)
            ldm_obs_embs = []
            for i in range(n_images): 
                raw_image_features=image[:, i*3:(i+1)*3, :, :]
                ldm_obs_emb =  self.get_obs_emb(raw_image_features=raw_image_features, raw_low_dim_data=batch.get('low_dim'))
                ldm_obs_embs.append(ldm_obs_emb)
            ldm_obs_emb = torch.cat(ldm_obs_embs, dim=1)
            ldm_obs_emb = self.obs_down(ldm_obs_emb).unsqueeze(1)
        ...
        # table = wandb.Table()
        # table.add_data
        # self.logger.experiment.log({'embedding_table': table})
        language_emb = self.get_language_emb(batch['text'])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            raw_action = batch['actions']
            pred_action = self.predict_action(
                raw_language_features=batch['text'],
                raw_image_features = batch['image'],
                raw_low_dim_data=batch.get('low_dim')
            )
            model_mse_error = F.mse_loss(raw_action.squeeze(-1), pred_action)
